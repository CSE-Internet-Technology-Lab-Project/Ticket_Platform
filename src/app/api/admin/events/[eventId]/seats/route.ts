...
export async function PATCH(request: Request, context: { params: Promise<{ eventId: string }> }) {
  ...
  const updated = await prisma.ticketInventory.updateMany({ where: { id: inventoryId, eventId, event: { organizerId: organizer.id }, status: status === "BLOCKED" ? "AVAILABLE" : "BLOCKED" }, data: { status, version: { increment: 1 } } });
